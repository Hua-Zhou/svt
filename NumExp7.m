%% comparison of deflation and succession method 
% singular value thresholding on sparse matrix
clear;
...
records = Sim(bfwb398,'choice','succession','rep',rep,'seed',seed);
records1(:,1) = records(:,1); % collect svt results
records2(:,1) = records(:,2); % collect objective function results

records = Sim(rdb800l,'choice','succession','rep',rep,'seed',seed);
records1(:,2) = records(:,1);
records2(:,2) = records(:,2);

records = Sim(tols1090,'choice','succession','rep',rep,'seed',seed);
records1(:,3) = records(:,1);
records2(:,3) = records(:,2);

records = Sim(mhd4800b,'choice','succession','rep',rep,'seed',seed);
records1(:,4) = records(:,1);
records2(:,4) = records(:,2);

records = Sim(cryg10000,'choice','succession','rep',rep,'seed',seed);
records1(:,5) = records(:,1);
records2(:,5) = records(:,2);

% save data
save('ne7_svt.mat','records1');
save('ne7_obj.mat','records2');
...
% legend(findobj(gca,'Tag','Box'),'succession','deflation','Location','northwest');
% set(gca,'fontsize',20);